...
while (timeElapsed <= maxTime) && ~captured
    ...
    if captured==true
        title(['preySpeed:',num2str(preySpeed),',predatorSpeed:',num2str(predatorSpeed),',Captured at ',num2str(timeElapsed),' timeSteps']);
        pause(5);
    else
        title(['preySpeed:',num2str(preySpeed),',predatorSpeed:',num2str(predatorSpeed),',timeSteps:',num2str(timeElapsed)]);
    end
